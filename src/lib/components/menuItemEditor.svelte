<script lang="ts">
  ...
  const updateValues = async () => {
    await fetch(
      `${APIUrl}/api/items/${itemId}/edit?${new URLSearchParams({
        name: name,
  ...
  const deleteMenuItem = async () => {
    await fetch(`${APIUrl}/api/items/${itemId}/delete`, {
      method: "POST",
...
<Card>
  <div class="flex flex-row justify-between items-center">
    <select
      ...
      class="rounded-sm text-neutral-900 dark:text-white pl-2"
      value={section}
      onchange={(event) => {
