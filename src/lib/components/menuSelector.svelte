<script lang="ts">
  ...
  const handleMenuChange = async () => {
    await fetch(
      `${APIUrl}/api/menu/${menuId}/edit?${new URLSearchParams({
        name: name,
...
<Card>
  <div class="flex flex-row justify-between">
    <a
      ...
      class="menu-title flex flex-col sm:flex-row grow items-center"
    >
      <div
        class="menu-name text-center lg:text-left text-2xl text-black dark:text-white"
      >
        {name}
      </div>
  ...
  <div class="menu-editor {editorOpen ? '' : 'hidden'} flex-col">
    <div>
      ...
      <input
        ...
        id="edit-menu-date"
        name="date"
        class="block date-editor border-1 border-neutral-700 rounded-md"
