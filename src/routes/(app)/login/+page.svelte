...
<div class="flex items-center h-full welcome-dialog">
  <div>
    ...
    <div
      class="bg-white dark:bg-zinc-800 border border-zinc-700 access-code-input rounded-md text-center"
    >
      {#if !notificationsPanelOpen}
        ...
        <form class="px-8 flex flex-row gap-x-3" onsubmit={handleSubmitCode}>
          <input
            ...
            style="border: 2px solid {statusBorder};"
          />
          <input
            type="submit"
            value="Go"
      ...
      {:else}
        <div class="flex flex-col gap-y-8 py-8 px-4">
          <h2 class="text-xl">
            In order to deliver your menus, we need your permission to send you
            a notification when that happens.
          </h2>
          <button
            onclick={subscribeUser}
            class="flex justify-center object-none text-7xl cursor-pointer rounded-md px-4 py-3"
            ><Bell id="bell-icon" /></button
          >
        </div>
      {/if}
    </div>
